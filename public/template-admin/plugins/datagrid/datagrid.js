  ...
  $.fn.datagrid = function (options) {
    ...
    var config = $.extend(
      {
        ...
        firstText: '<i class="fa fa-chevron-left"></i>',
        prevText: '<i class="fa fa-chevron-left"></i>',
        nextText: '<i class="fa fa-chevron-right"></i>',
        lastText: '<i class="fa fa-chevron-right"></i>',
    ...
    function PrintMergeCells() {
      ...
      if (config.rowNumber == true) {
        var rowNumberString =
          "<th sortable='false' style='text-align: center; width: 20px;' rowspan='2'>No</th>";
      ...
      if (config.rowCheck == true) {
        var rowCheckString =
          "<th sortable='false' style='text-align: center; width: 20px;' rowspan='2'><input type='checkbox'></th>";
      ...
      if (!$.isEmptyObject(config.rowDetail)) {
        var rowDetail =
          "<th sortable='false' style='text-align: center; width: 20px;' rowspan='2'></th>";
      ...
      config.columns.forEach(function (row_column, i) {
        ...
        if (print_merge_thead) {
          columnString =
            "<th sortable='false' style='text-align: " +
            align +
            ";' colspan='" +
            colspan +
            "'>" +
    ...
    function PrintNormalCells() {
      ...
      if (config.rowNumber == true && config.mergeCells.length == 0) {
        var rowNumberString =
          "<th sortable='false' style='text-align: center; width: 20px;'>No</th>";
      ...
      if (config.rowCheck == true && config.mergeCells.length == 0) {
        var rowCheckString =
          "<th sortable='false' style='text-align: center; width: 20px;'><input type='checkbox'></th>";
      ...
      if (!$.isEmptyObject(config.rowDetail) && config.mergeCells.length == 0) {
        var rowDetail =
          "<th sortable='false' style='text-align: center; width: 20px;'></th>";
    ...
    function DisplayData(activePageNumber, itemsPerPage, dataSearch) {
      ...
      data = AjaxRequest(
        ...
        function () {
          var pageNumber =
            parseInt(activePageNumber) * itemsPerPage - itemsPerPage + 1;
          var temp = "";
          var rowData = "";
          var rowIndex = "";
          ...
          if (data.total >= 1) {
            for (var i = 0; i < data.rows.length; i++) {
              ...
              if (config.rowCheck) {
                temp +=
                  "<td class='checkbox-column' style='text-align: center;'><input value='" +
                  data.rows[i][config.primaryField] +
                  "' type='checkbox'></td>";
              }
              ...
              if (!$.isEmptyObject(config.rowDetail)) {
                temp +=
                  ...
                  "<div class='detail-link' data-id='" +
                  i +
                  "' data-expand='false' style='width: 10px; height: 4px; margin-left: auto; margin-right: auto; cursor: pointer; margin-top: 8px; background-color: #595959; border: 1px solid #595959; -border-radius: 0.1em; -moz-border-radius: 0.1em; -webkit-border-radius: 0.1em;'>" +
              ...
              config.columns.forEach(function (rowColumn) {
                if (typeof data.rows[i][rowColumn.field] !== "undefined") {
                  ...
                  temp +=
                    ...
                    rowColumn.class +
                    "'>" +
                    rowColumn.rowStyler(data.rows[i], i) +
                    "</td>";
                } else {
                  temp +=
                    "<td style='text-align: " +
    ...
    function DisplayChart(data) {
      ...
      if (data.rows.length > 0) {
        ...
        var temparr = {};
        eval(
          "temparr." + config.chartxkey + ' = "' + config.noRecordText + '"'
        );
        $.each(config.chartykey, function (index, val) {
          var chartykey = this;
    ...
    function PagingData(activePageNumber, itemsPerPage) {
      ...
      temp +=
        "<li " +
        first_link_disabled +
        ' title="first"><a href="javascript:void(0);" class="btn btn-primary mr-1">' +
        config.firstText +
        "</a></li>";
      temp +=
        "<li " +
        first_link_disabled +
        ' title="prev"><a href="javascript:void(0);" class="btn btn-primary mr-1">' +
        config.prevText +
        "</a></li>";

      if (
        activePageNumber == Math.ceil(data.total / itemsPerPage) &&
        activePageNumber - 2 >= 1
      ) {
        temp +=
          '<li title="' +
          (activePageNumber - 2) +
      ...
      temp +=
        "<li " +
        last_link_disabled +
        ' title="next"><a href="javascript:void(0);" class="btn btn-primary mr-1">' +
        config.nextText +
        "</a></li>";
      temp +=
        "<li " +
        last_link_disabled +
        ' title="last"><a href="javascript:void(0);" class="btn btn-primary mr-1">' +
        config.lastText +
        "</a></li>";
            ...
            $(this).on("click", function () {
              ...
              if (isDisabled != true) {
                if (this.title == "prev" && activePageNumber - 1 >= 1) {
                  ...
                  this.title == "next" &&
                  activePageNumber + 1 <= Math.ceil(data.total / itemsPerPage)
                ) {
                  DisplayData(
                    activePageNumber + 1,
                    itemsPerPage,
                  ...
                  parseInt(this.title) <= activePageNumber + 2 ||
                  parseInt(this.title) >= activePageNumber - 2
                ) {
                  DisplayData(
                    parseInt(this.title),
    ...
    function OptionPaging() {
      ...
      $(config.optionPagingElement).on("change", function () {
        config.itemsPerPage = $(config.optionPagingElement)
          ...
          $(this)
            ...
            .each(function (index, object) {
              ...
              $(this).on("click", function () {
                ...
                if (!found) {
                  config.rowChecked[config.rowChecked.length] = parseInt(
                    this.value
                  );
                } else {
      ...
      config.columns.forEach(function (rowColumn, key) {
        if (rowColumn.search) {
          ...
          if (rowColumn.custom_search == undefined) {
            $(config.searchFieldElement).append(
              '<option value="' +
                rowColumn.field +
                '">' +
                rowColumn.title +
    ...
    function CustomSearch(el) {
      ...
      if (costum_search == undefined) {
        $(config.searchInputElement).replaceWith(
          searchInputElementOriginal.element
        );
        $(config.searchInputElement).on("keyup", function () {
          delay(function () {
            DisplayData(1, config.itemsPerPage, SearchInput());
        ...
        config.columns.forEach(function (rowColumn, key) {
          if (rowColumn.field == column) {
            if (searchInputElementOriginal.selector.indexOf(".") == -1) {
              ...
              $(select).attr(
                "class",
                rowColumn.custom_search.appendClass + " " + selector
              );
            }
      ...
      if (data.total >= 1) {
        ...
        if (activePageNumber == Math.ceil(data.total / itemsPerPage)) {
          offset =
            ...
            (activePageNumber * itemsPerPage - data.total);
        } else {
          offset = activePageNumber * itemsPerPage;
        }

        $(config.pageInfoElement).html(
          config.showingText +
            " " +
            limit +
            " - " +
          ...
          $(this).on("click", function () {
            ...
            if ($(this).attr("data-expand") == "false") {
              ...
              var temp =
                "<div class='detail-link' data-id='" +
                rowIndex +
                "' data-expand='true' style='width: 10px; height: 4px; margin-left: auto; margin-right: auto; cursor: pointer; margin-top: 8px; background-color: #595959; border: 1px solid #595959; -border-radius: 0.1em; -moz-border-radius: 0.1em; -webkit-border-radius: 0.1em;'></div>";
    ...
    function rowModifier(row, columnIndex, rowIndex, styler, onEdit, onSave) {
      $(row)
        .children()
        .each(function (child_index, child_object) {
          if (columnIndex == "all") {
