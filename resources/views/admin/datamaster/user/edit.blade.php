            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">Edit Data {{ $judul }}</li>
                    </ol>
                </div>
            </div>
        </div><!-- /.container-fluid -->
                ...
                <div class="col-md-7 mx-auto">
                    ...
                    <div class="card card-primary">
                        <div class="card-header">
                            <h3 class="card-title">Form Edit {{ $judul }}</h3>
                        </div>
                        <!-- /.card-header -->
                        <!-- form start -->
                            ...
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="email" name="email"
                                        ...
                                        value="{{ old('email') ? old('email') : $users->email }}" required>
                                </div>
                                <div class="form-group">
                                    <label for="password">
                                        Password <sup class="text-danger">*</sup> Isi Password Jika Ingin Menggantinya
                                    </label>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="status" id="status" class="form-control" required>
                                        ...
                                        <option value=" {{ old('status') ? old('status') : 'Aktif' }} "
                                            {{ $users->status == 'Aktif' ? 'selected' : '' }}>Aktif
                                        </option>
                                        <option value="{{ old('status') ? old('status') : 'Non-Aktif' }}"
                                            {{ $users->status == 'Non-Aktif' ? 'selected' : '' }}>
                                            Non-Aktif</option>
                                    </select>
                                </div>
                                <div class="form-group">
                                    ...
                                    <textarea name="alamat_lengkap" id="alamat_lengkap" cols="30" rows="3" class="form-control"
                                        placeholder="Masukkan Alamat Lengkap" required>{{ $users->alamat_lengkap }}</textarea>
                                </div>
                ...
                $.post("{{ route('usersGetKabupaten') }}", {
                    ...
                    if (data.status == 'success') {
                        ...
                        data.data.forEach((v, i) => {
                            ...
                            html +=
                                `<option value="{{ old('kabupaten_id') ? old('kabupaten_id') : '${v.id_kabupaten }' }}" ${(v.id_kabupaten == "{{ $users->kabupaten_id }}") ? 'selected' : ''}>${v.nama_kabupaten}</option>`
                        })
